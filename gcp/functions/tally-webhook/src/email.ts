 ...
 * TODO(cloud-tasks): replace with Cloud Tasks enqueue for durable retries:
 *   enqueueEmailTask({ auditId, publicId }) → worker reads row and sends.
 */
...
/** Stub for durable async dispatch (Cloud Tasks). */
export function enqueueEmailTaskStub(auditId: string, publicId: string): void {
  // TODO: Cloud Tasks HTTP target → /internal/send-audit-email
  console.info(
    JSON.stringify({
      msg: "email_task_stub",
      auditId,
      publicId,
      note: "Using fire-and-forget sendReportEmail until Cloud Tasks is wired",
    }),
  );
}
