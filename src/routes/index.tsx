    ...
    <SiteShell>
      <section className="mx-auto max-w-6xl px-4 pb-8 pt-10 sm:px-6 sm:pt-16">
        <p className="text-xs font-medium uppercase tracking-[0.2em] text-primary">
          Field ledger · Northeast
        </p>
        <h1 className="mt-4 max-w-3xl font-display text-4xl leading-[1.1] tracking-tight text-fg sm:text-6xl">
      ...
      <section className="mx-auto max-w-6xl px-4 sm:px-6">
        <figure className="overflow-hidden rounded-xl border border-border bg-surface">
          <img
            src="/images/hero.jpg"
            alt="Rural Northeast property with a broad lawn and wood line"
            className="aspect-video w-full object-cover"
            crossOrigin="anonymous"
          />
          <figcaption className="flex items-center justify-between gap-4 px-4 py-3 text-xs text-muted sm:px-5">
      ...
      <section className="mx-auto mt-10 max-w-6xl px-4 sm:px-6">
        <div className="grid gap-px overflow-hidden rounded-xl border border-border bg-border sm:grid-cols-3">
          {[
            ...
            <div key={item.l} className="bg-surface px-5 py-6">
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {STEPS.map((step) => (
            <article key={step.n} className="rounded-lg border border-border bg-surface p-5">
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
        <div className="flex items-end justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 font-display text-3xl tracking-tight">Services on the lot</h2>
          </div>
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          {SERVICES.map((service) => (
            ...
            >
              ...
              <p className="mt-4 text-xs text-subtle">{service.turnaround}</p>
            </article>
          ))}
        </div>
      </section>
