...
5. Fast ack: validate → insert → **200**; then email async (fire-and-forget). Durable queue is a **stub only** (`enqueueEmailTaskStub` logs intent). If the function instance dies mid-send, there is **no durable retry** until a Background Function / queue is wired. Unscored service-log emails are **intake alerts** (address / tank / gallons / notes / photo refs), **not** scored briefings.
